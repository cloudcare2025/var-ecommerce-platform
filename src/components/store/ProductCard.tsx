  ...
  return (
    <div className="bg-white rounded-lg border border-[#E2E8F0] overflow-hidden group hover:-translate-y-1 hover:shadow-xl transition-all duration-300 flex flex-col">
      ...
      <Link
        ...
        className="gradient-blue-soft p-8 flex items-center justify-center min-h-[200px] relative"
      >
        {/* Category badge top-left */}
        <span className="absolute top-3 left-3 bg-[#0075DB]/10 text-[#0075DB] text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide">
      ...
      <div className="p-5 flex-1 flex flex-col">
        {product.series && (
          <p className="text-[10px] font-bold tracking-[0.1em] uppercase text-[#0075DB] mb-1">
            {product.series}
          </p>
        )}
        <Link href={`/products/${product.slug}`}>
          <h3 className="font-heading text-lg font-semibold mb-1.5 text-[#1F2929] hover:text-[#0075DB] transition-colors line-clamp-2 leading-snug">
            {product.name}
          </h3>
        </Link>
          ...
          <div>
            {product.msrp > 0 ? (
              <>
                ...
                <span className="font-heading text-lg font-bold text-[#1F2929]">
