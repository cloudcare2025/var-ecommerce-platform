...
import { mockBrands, mockProducts, mockOrders, mockCustomers } from "@/lib/mock-data";
...
export default function BrandDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const brandProducts = mockProducts.filter((p) => p.brands.includes(brand.slug));
  const brandOrders = mockOrders.filter((o) => o.brandSlug === brand.slug);
  const brandCustomers = mockCustomers.filter((c) => c.brandSlug === brand.slug);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="bg-admin-card rounded-xl border border-admin-border p-4">
          ...
          <div className="text-2xl font-semibold text-admin-text">{brandProducts.length}</div>
        </div>
        <div className="bg-admin-card rounded-xl border border-admin-border p-4">
          <div className="flex items-center gap-2 text-sm text-admin-text-muted mb-1">
        ...
        <div className="bg-admin-card rounded-xl border border-admin-border p-4">
          ...
          <div className="text-2xl font-semibold text-admin-text">{brandCustomers.length}</div>
        </div>
      </div>
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="bg-admin-card rounded-xl border border-admin-border">
            <div className="px-6 py-4 border-b border-admin-border flex items-center justify-between">
              ...
              <span className="text-sm text-admin-text-muted">{brandProducts.length} products</span>
            </div>
