...
export default function InventoryPage() {
  ...
  const inventoryItems = useMemo(() => {
    return mockProducts.map((product) => {
      ...
      // Simulated distributor stock (would come from API in production)
      const distributorStock = Math.max(0, product.stock * 3 + Math.floor(Math.random() * 20));
      return {
        ...product,
        stockInfo,
        distributorStock,
  ...
  const lowStockCount = inventoryItems.filter(
    (i) => i.stockInfo.status === "low-stock"
  ).length;
  const outOfStockCount = inventoryItems.filter(
    (i) => i.stockInfo.status === "out-of-stock"
  ).length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="bg-admin-card rounded-xl border border-admin-border p-4">
          ...
          <div className="text-2xl font-semibold text-admin-text">{inventoryItems.length}</div>
        </div>
        <div className="bg-admin-card rounded-xl border border-admin-border p-4">
          <div className="flex items-center gap-2 text-sm text-admin-text-muted mb-1">
            <Warehouse size={14} />
          ...
          <div className="text-2xl font-semibold text-admin-text">{totalUnits}</div>
        </div>
        <div className="bg-admin-card rounded-xl border border-admin-border p-4">
          ...
          <div className="text-2xl font-semibold text-admin-warning">{lowStockCount}</div>
        </div>
        <div className="bg-admin-card rounded-xl border border-admin-border p-4">
          ...
          <div className="text-2xl font-semibold text-admin-danger">{outOfStockCount}</div>
        </div>
      </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700 mt-1">
                {outOfStockCount > 0 && (
                  <span className="font-medium">{outOfStockCount} product{outOfStockCount > 1 ? "s" : ""} out of stock. </span>
                )}
                {lowStockCount > 0 && (
                  <span>{lowStockCount} product{lowStockCount > 1 ? "s" : ""} below low stock threshold.</span>
                )}
              </p>
